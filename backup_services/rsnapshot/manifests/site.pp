...
#A machine to monitor rsnapshot backups from other machines
node 'rsnapshotmonitor.local' {
...
#Ubuntu 14.04 rsnapshot server node
node 'rsnapshotserver1.local' {
...
#CentOS 6 rsnapshot server node
node 'rsnapshotserver1.local' {
...
#CentOS 7 rsnapshot server node
node 'rsnapshotserver3.local' {
...
#Ubuntu 14.04 rsnapshot client node
node 'rsnapshotclient1.local' {
...
node 'rsnapshotlogging.local' {
  ...
  #Install Postfix so we can monitor SMTP services and send out email alerts from logs coming through Logstash:
  class { '::postfix::server':
    inet_interfaces => 'all', #Listen on all interfaces
    inet_protocols => 'all', #Use both IPv4 and IPv6
    mydomain       => 'local',
  ...
  ::apache::vhost { 'kibana.rsnapshotlogging.local_non-ssl':
    port            => 80,
